...
Drizzle supports a raw representation of Postgres policies and roles that can be used in any way you want, even with popular Postgres database providers such as `Neon` and `Supabase`.

In Drizzle, we have specific places for predefined RLS roles and functions for both database providers, but you can also define your own logic.
...
If you add a policy to a table, RLS will be enabled automatically, so there’s no need to explicitly enable RLS.
</Callout>
...
If you are using drizzle-kit to manage your schema, particularly for roles, there may be situations where roles are not defined in your Drizzle schema. In such cases, you may want drizzle-kit to skip managing these roles without you having to define each one in your drizzle schema and marking it with `.existing()`.
